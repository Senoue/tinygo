...
// The ESP32-C5 has the same built-in USB Serial/JTAG hardware IP as the
// ESP32-C3 and ESP32-C6. The peripheral clock is enabled via the PCR
// (Peripheral Clock Reset) block, like on the C6.

const cpuInterruptFromUSB = 10

type USB_DEVICE struct {
	Bus       *esp.USB_SERIAL_JTAG_Type
	Buffer    *RingBuffer
...
// USBDevice provides a stub USB device for the ESP32-C5. The hardware
...
func (usbdev *USB_DEVICE) Configure(config UARTConfig) error {
	...
	esp.INTERRUPT_CORE0.SetUSB_SERIAL_JTAG_INTR_MAP(cpuInterruptFromUSB + clicExtIntrNumOffset)
...
func (usbdev *USB_DEVICE) handleInterrupt() {
	...
	if intStatus&esp.USB_SERIAL_JTAG_INT_ST_SERIAL_OUT_RECV_PKT_INT_ST != 0 {
...
// ConfigureUSBEndpoint is a no-op on ESP32-C5 — the hardware does not
// support programmable USB endpoints.
func ConfigureUSBEndpoint(desc descriptor.Descriptor, epSettings []usb.EndpointConfig, setup []usb.SetupConfig) {
}

// SendZlp is a no-op on ESP32-C5.
func SendZlp() {
}

// SendUSBInPacket is a no-op on ESP32-C5.
